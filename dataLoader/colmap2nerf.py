...
def run_colmap(args):
	...
	if os.path.exists(db):
		os.remove(db)
	do_system(f"colmap feature_extractor --SiftExtraction.use_gpu 0 --SiftExtraction.num_threads 14 --ImageReader.camera_model OPENCV --SiftExtraction.estimate_affine_shape=true --SiftExtraction.domain_size_pooling=true --ImageReader.single_camera 1 --database_path {db} --image_path {images}")
	do_system(f"colmap {args.colmap_matcher}_matcher --SiftMatching.use_gpu 0 --SiftMatching.num_threads 14 --SiftMatching.guided_matching=true --database_path {db}")
	try:
